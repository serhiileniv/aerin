...
`src/providers/catalog.ts`'s `PROVIDER_CATALOG` is only the "featured" shortcut on top of that — providers common enough to deserve a name and a preset `baseURL` instead of the custom-endpoint flow. It's small and hand-picked on purpose (currently: the 5 native SDKs, major OpenAI-compatible platforms, several Chinese providers — Alibaba/Qwen, SiliconFlow, Volcengine/Doubao, StepFun, SenseNova, Tencent Hunyuan, MiniMax — direct vendor entries for NVIDIA/Nemotron, Xiaomi/MiMo, and Meta/Muse, and OpenCode Zen, a hosted gateway with several genuinely free-tier models); everything else routes through layers 2 and 3 automatically.

Note on "free": a provider-level `freeTier` flag (used for Google/Groq/Cerebras) only suppresses cost *display* for that provider — it's not a per-model guarantee. Google's own free tier, for example, is zero for some preview models (`computer-use-preview`, `antigravity-preview`) even though the flag is set on the provider; those two are filtered out of the picker entirely (`list-models.ts`) since they fail any real chat turn regardless of billing. OpenCode Zen takes the more accurate approach instead: no provider-level flag, just real per-model $0/$0 pricing synced from models.dev, so the picker shows "free" only on the ids that actually are.
